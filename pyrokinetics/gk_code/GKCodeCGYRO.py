...
class GKCodeCGYRO(GKCode):
    ...
    def write(self, pyro, file_name, directory="."):
        ...
        if numerics.nonlinear:
            ...
            cgyro_input["BOX_SIZE"] = int(
                (numerics.ky * 2 * pi * shat / numerics.kx) + 0.1
            )
        else:
        ...
        self.to_file(
            cgyro_input, file_name, directory=directory, float_format=pyro.float_format
        )
    ...
    def load_grids(self, pyro):
        ...
        theta_ballooning = grid_data[
            starting_point : starting_point + ntheta_ballooning
        ]
